...
lora_send_packet(uint8_t *buf, int size)
{
   ...
   if ((irq & IRQ_TX_DONE_MASK) == IRQ_TX_DONE_MASK) break;
   loop++;
   if (loop >= 80) break;  // Tăng số vòng lặp lên để chờ thêm thời gian
   vTaskDelay(pdMS_TO_TICKS(10));  // Tăng độ trễ lên 5ms thay vì 2ms
}
   if (loop >= 80) {
   __send_packet_lost++;
   ESP_LOGE(TAG, "lora_send_packet Fail");
...
#endif
   ESP_LOG_BUFFER_HEX("LoRa Packet", buf, len);
   printf("Hello 0x%02x 0x%02x\n", buf[0], buf[1]);
